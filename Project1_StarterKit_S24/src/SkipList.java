...
public class SkipList<K extends Comparable<? super K>, V>
    ...
    public void PrintElements(SkipNode list) {
        SkipNode temp = list;
        while (temp != null) {
            System.out.printf("%s %d -> \t",temp.pair.getKey().toString(),temp.level);
            temp = temp.forward[0];
        }
    }
        ...
        for (int i = new_level ; i >= 0; i--) {
            ...
            while (true) {
                if (temp.forward[i] != null) {
                    if(temp.forward[i].pair.getKey().compareTo(it.getKey()) > 0) {
                        TempList[i] = temp;
                        break;
                    }else {
                        temp = temp.forward[i];
    ...
    public void adjustHead(int newLevel) {
        SkipNode[] temp1_list = (SkipNode[])Array.newInstance(SkipList.SkipNode.class,
            newLevel);
